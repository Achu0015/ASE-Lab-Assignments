...
var app = express();
var result={'body': []};
var url = 'mongodb://Lab10:achu10@ds051838.mlab.com:51838/aselab';
app.use(cors());
...
app.post('/update',function (req,res) {
    ...
    var updateDocument = function(db, data, callback) {
        db.collection('Table1').updateOne({"_id":ObjectId(id)},{$set:item}, function(err, result) {
            if(err)            {
                res.write("Registration Failed");
                res.end();            }
...
app.post('/delete', function(req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        if(err)        {
            res.write("Registration Failed");
            res.end();        }
        db.collection('Table1').deleteOne({"_id": ObjectId(id)}, function(err, result) {
